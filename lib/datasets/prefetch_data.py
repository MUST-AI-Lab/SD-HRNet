...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_meta = next(self.loader)
...
class mi_data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_input1, self.next_input2, self.next_target1, self.next_meta1 = next(self.loader)
        except StopIteration:
            self.next_input1 = None
            self.next_input2 = None
            self.next_target1 = None
            self.next_meta1 = None
            return
        # if record_stream() doesn't work, another option is to make sure device inputs are created
        # on the main stream.
        # self.next_input_gpu = torch.empty_like(self.next_input, device='cuda')
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_input1 = self.normalize(self.next_input1)
            if self.is_cutout:
                self.next_input1 = self.cutout(self.next_input1)

            self.next_input2 = self.next_input2.cuda(non_blocking=True)
            self.next_input2 = self.normalize(self.next_input2)
            if self.is_cutout:
                self.next_input2 = self.cutout(self.next_input2)
    ...
    def next(self):
        ...
        self.preload()
        return input1, input2, target1, meta1
