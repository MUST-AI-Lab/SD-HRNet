...
OPS = {
    ...
    'basic_block_v2': lambda C_in, C_out, stride, affine, track_running_stats: BasicBlockV2(C_in, C_out, stride, affine=affine, track_running_stats=track_running_stats),
    'basic_block_se': lambda C_in, C_out, stride, affine, track_running_stats: SEBasicBlock(C_in, C_out, stride, affine=affine, track_running_stats=track_running_stats),
    ...
    'conv_3x3': lambda C_in, C_out, k_size, stride, padding, affine, track_running_stats: Conv(C_in, C_out, k_size, stride, padding, affine=affine, track_running_stats=track_running_stats),
    'conv_1x1': lambda C_in, C_out, k_size, stride, padding, affine, track_running_stats: Conv(C_in, C_out, k_size, stride, padding, affine=affine, track_running_stats=track_running_stats),
    'sep_conv_3x3': lambda C_in, C_out, stride, affine, track_running_stats: SepConv(C_in, C_out, 3, stride, 1, affine=affine, track_running_stats=track_running_stats),
    'sep_conv_5x5' : lambda C_in, C_out, stride, affine, track_running_stats: SepConv(C_in, C_out, 5, stride, 2, affine=affine, track_running_stats=track_running_stats),
    'sep_conv_7x7' : lambda C_in, C_out, stride, affine, track_running_stats: SepConv(C_in, C_out, 7, stride, 3, affine=affine, track_running_stats=track_running_stats),
    'dil_conv_3x3' : lambda C_in, C_out, stride, affine, track_running_stats: DilConv(C_in, C_out, 3, stride, 2, 2, affine=affine, track_running_stats=track_running_stats),
    'dil_conv_5x5' : lambda C_in, C_out, stride, affine, track_running_stats: DilConv(C_in, C_out, 5, stride, 4, 2, affine=affine, track_running_stats=track_running_stats),
    'conv_7x1_1x7' : lambda C_in, C_out, stride, affine, track_running_stats: nn.Sequential(
...
class Conv(nn.Module):
    # relu will be applied after fusion
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True, track_running_stats=True):
        super(Conv, self).__init__()
        self.op = nn.Sequential(
            nn.Conv2d(C_in, C_out, kernel_size=kernel_size, stride=stride, padding=padding, bias=False),
            nn.BatchNorm2d(C_out, momentum=0.01, affine=affine, track_running_stats=track_running_stats),
        )
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, groups=1,
        ...
        self.bn1 = norm_layer(planes, momentum=0.01, affine=affine, track_running_stats=track_running_stats)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes, momentum=0.01, affine=affine, track_running_stats=track_running_stats)
        self.downsample = None
        if stride != 1 or inplanes != planes:
            self.downsample = nn.Sequential(
                conv1x1(inplanes, planes, stride),
                norm_layer(planes, momentum=0.01, affine=affine, track_running_stats=track_running_stats),
            )
...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, affine=True, track_running_stats=True):
        ...
        self.conv1 = nn.Conv2d(inplanes, inter_planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(inter_planes)
        self.conv2 = nn.Conv2d(inter_planes, inter_planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(inter_planes)
        self.conv3 = nn.Conv2d(inter_planes, planes, kernel_size=1, bias=False)# planes * 4
        self.bn3 = nn.BatchNorm2d(planes)# planes * 4
        self.relu = nn.ReLU(inplace=True)
        self.stride = stride
        self.downsample = None
...
class BasicBlockV2(nn.Module):
    def __init__(self, inplanes, planes, stride=1, groups=1,
        ...
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock")
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.bn1 = norm_layer(inplanes, affine=affine, track_running_stats=track_running_stats)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = conv3x3(inplanes, planes, stride)
